 ...
 * - responseHandler(request, response, SAMLRequest, next) a function that handles the response. Defaults to HTML POST to postUrl.
 *
  ...
  function execute (postUrl, audience, req, res, next) {
    ...
    getSamlResponse(opts, user, function (err, SAMLResponse) {
      ...
      if (opts.responseHandler) {
        opts.responseHandler(req, res, response, next);
      } else {
